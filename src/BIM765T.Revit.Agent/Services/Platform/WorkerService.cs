...
internal sealed class WorkerService
{
    ...
    // ── Internal accessors for workflow steps (no new surface area) ────
    internal WorkerReasoningEngine Reasoning => _reasoning;
    internal LlmResponseEnhancer? Enhancer => _enhancer;
    internal PersonaRegistry Personas => _personas;
    internal MissionCoordinator Missions => _missions;
    ...
    internal WorkerResponse HandleMessage(UIApplication uiapp, ToolRequestEnvelope envelope, WorkerMessageRequest request)
    {
        ...
        // Greeting, identity, help, context_query → skip LLM planner + tool chain + capability compile.
        // 1 LLM call (enhance) or rule-based fallback → immediate response.
        var quickClassification = _reasoning.ClassifyIntent(request.Message, session.PendingApprovalState?.HasPendingApproval ?? false);
        if (WorkerReasoningEngine.IsConversationalIntent(quickClassification.Intent))
        {
            return HandleConversationalFastPath(uiapp, doc, envelope, request, session, workspaceId, contextSummary, quickClassification);
        }
    ...
    // ── Conversational fast-path ─────────────────────────────────────
    // For greeting, identity_query, help, context_query.
    // Skips: LLM planner, playbook match, capability compile, tool chain, separate enhance.
    // Only: 1 LLM call (enhance) with compact prompt + Revit context, or rule-based fallback.
    private WorkerResponse HandleConversationalFastPath(
        UIApplication uiapp,
        Document doc,
        ...
        WorkerIntentClassification classification)
    {
        var decision = new WorkerDecision
        {
            ...
            Goal = $"Conversational: {classification.Intent}",
        ...
        // Build context-aware fallback text (used when LLM unavailable).
        string responseText;
        switch (classification.Intent)
        {
            ...
            case "help":
                ...
                break;
            case "context_query":
                responseText = BuildContextQueryResponseText(uiapp, doc, request.Message, workspaceId);
                break;
            default:
                responseText = BuildHelpResponseText(contextSummary);
                break;
        ...
        // Single LLM enhance call — replaces both planner LLM + enhance LLM.
        var narrationMode = WorkerNarrationModes.RuleOnly;
        var narrationDiagnostics = "Conversational fast-path, rule-based text.";
        if (_enhancer != null && _enhancer.IsLlmConfigured)
        {
            var persona = _personas.Resolve(session.PersonaId);
            var narration = _enhancer.EnhanceResponse(
                request.Message,
        ...
        {
            "greeting" => "Worker đã sẵn sàng cho nhiệm vụ tiếp theo.",
            "identity_query" => "Worker đã giới thiệu vai trò và lane hỗ trợ hiện tại.",
            "help" => "Worker đã gợi ý các điểm vào an toàn.",
            _ => "Đã tổng hợp context hiện tại."
        };
        _missions.Complete(session, completionNote);

        // Finalize context + session memory.
        contextSummary = BuildContextSummary(uiapp, doc, request.Message, classification.Intent);
        var queueState = GetQueueState();
        contextSummary.QueueSummary = BuildQueueSummary(queueState);
        contextSummary.WorkspaceId = workspaceId;
        _conversations.UpdateContextSummary(session.SessionId, contextSummary);
        var workerMessage = CreateMessage(WorkerMessageRoles.Worker, responseText);
        _conversations.AddMessage(session.SessionId, workerMessage);
        CaptureSessionMemory(session, WorkerMemoryKinds.WorkerResponse, responseText, doc, string.Empty, "worker", classification.Intent);

        if (ShouldPersistEpisode(session))
        {
            PersistEpisode(session, new List<WorkerToolCard>(), new List<string>(), responseText);
        }
        ...
        return new WorkerResponse
        {
            ...
            PlaybookPreview = new PlaybookPreviewResponse { WorkspaceId = workspaceId },
            StandardsSummary = string.Empty,
            ResolvedCapabilityDomain = CapabilityDomains.General,
            ...
            ContextPills = BuildContextPills(contextSummary, session, WorkerExecutionTiers.Tier0),
            ...
            OnboardingStatus = BuildOnboardingStatus(session, contextSummary, new PendingApprovalRef()),
            ...
            ResponseModel = decision.ResponseModel,
            ReasoningMode = "fast-path",
            NarrationMode = narrationMode,
            NarrationDiagnostics = narrationDiagnostics,
            AutonomyMode = session.Mission.AutonomyMode,
            PlannerTraceSummary = "Conversational fast-path, no planner invoked.",
            ChosenToolSequence = new List<string>()
        };
    }

    /// <summary>
    /// Lightweight context query response — doesn't run full grounded research pipeline.
    /// </summary>
    private string BuildContextQueryResponseText(UIApplication uiapp, Document doc, string message, string workspaceId)
    {
        var contextSummary = BuildContextSummary(uiapp, doc, message, "context_query");
        var parts = new List<string>
        {
            $"Document: {contextSummary.DocumentTitle}",
            $"Active view: {contextSummary.ActiveViewName}",
            $"Selection: {contextSummary.SelectionCount} phần tử"
        };
        if (!string.IsNullOrWhiteSpace(contextSummary.Summary))
        {
            parts.Add(contextSummary.Summary);
        }
        return string.Join(". ", parts) + ".";
    }

    // ── Async workflow pipeline phases ─────────────────────────────────
    // GatherContext (UI) → Plan (off-thread) → ExecuteIntent (UI) → Enhance (off-thread) → BuildResponse (UI)
    // These methods decompose HandleMessage into discrete phases for the async pipeline.
    // HandleMessage itself stays untouched as the synchronous single-call path.

    /// <summary>
    /// Phase 1 — Runs on UI thread. Resolves document, session, workspace and captures the
    /// initial user message into conversation history and session memory.
    /// </summary>
    internal MessageWorkflowContext GatherContextForWorkflow(UIApplication uiapp, ToolRequestEnvelope envelope, WorkerMessageRequest request)
    {
        request ??= new WorkerMessageRequest();
        var doc = ResolveDocument(uiapp, envelope);
        var documentKey = _platform.GetDocumentKey(doc);
        ...
        return new MessageWorkflowContext
        {
            Envelope = envelope,
            Request = request,
            Doc = doc,
    ...
    /// <summary>
    /// Phase 3 — Runs on UI thread. Applies planner hints, ensures mission, resolves capability
    /// planning, and dispatches to the appropriate intent handler. Reads <see cref="MessageWorkflowContext.Decision"/>
    /// (populated by the async PlanStep) instead of computing it inline.
    /// Does NOT include LLM narration/enhancement — that is handled by the async EnhanceStep.
    /// </summary>
    internal void ExecuteIntentForWorkflow(UIApplication uiapp, MessageWorkflowContext ctx)
    {
        ...
        var contextSummary = ctx.ContextSummary;
        ...
        // Playbook prefix for non-trivial intents that matched a playbook
        if (!string.IsNullOrWhiteSpace(selectedPlaybook.PlaybookId)
            && !string.Equals(decision.Intent, "greeting", StringComparison.OrdinalIgnoreCase)
            && !string.Equals(decision.Intent, "help", StringComparison.OrdinalIgnoreCase)
            && !string.Equals(decision.Intent, "view_authoring_request", StringComparison.OrdinalIgnoreCase)
        ...
        // Populate ctx — LLM enhancement is handled separately by EnhanceStep
        ctx.ContextSummary = contextSummary;
        ctx.ResponseText = responseText;
        ctx.ToolCards = toolCards;
        ctx.ActionCards = actionCards;
        ctx.ArtifactRefs = artifactRefs;
        ctx.SelectedPlaybook = selectedPlaybook;
        ctx.PlaybookPreview = playbookPreview;
        ctx.CompiledPlan = compiledPlan;
        ctx.CapabilityDomain = capabilityDomain;
    }

    /// <summary>
    /// Phase 5 — Runs on UI thread. Assembles the final <see cref="WorkerResponse"/> from all
    /// accumulated context, adds the worker message to conversation, captures session memory,
    /// and persists the episode if eligible.
    /// </summary>
    internal WorkerResponse BuildResponseForWorkflow(UIApplication uiapp, MessageWorkflowContext ctx)
    {
        var doc = ctx.Doc!;
        var request = ctx.Request;
        var decision = ctx.Decision;
        ...
        var workspaceId = ctx.WorkspaceId;
        var contextSummary = ctx.ContextSummary;
        var responseText = ctx.ResponseText;
        var toolCards = ctx.ToolCards;
        var actionCards = ctx.ActionCards;
        var artifactRefs = ctx.ArtifactRefs;
        var selectedPlaybook = ctx.SelectedPlaybook;
        var playbookPreview = ctx.PlaybookPreview;
        var compiledPlan = ctx.CompiledPlan;
        var narrationMode = ctx.NarrationMode;
        var narrationDiagnostics = ctx.NarrationDiagnostics;

        // Final context summary pass (skip if already refreshed by project research)
        if (!string.Equals(decision.Intent, "context_query", StringComparison.OrdinalIgnoreCase)
            && !string.Equals(decision.Intent, "project_research_request", StringComparison.OrdinalIgnoreCase))
        {
            contextSummary = BuildContextSummary(uiapp, doc, request.Message, decision.Intent);
